    ...
    def start(self, n_episodes):
        ...
        for run_id, params in enumerate(self._logger.get_run_params()):
            ...
            for episode_idx in range(n_episodes):
                ...
                # NEW: Track which agents have completed for team rewards
                ...
                for step in range(self._max_steps - 1):
                    ...
                    # ============================================================
                    # IMPROVED REWARD SHAPING WITH COORDINATION REWARDS
                    # ============================================================
                    train_time = time.time()
                    
                    # Pre-compute agent positions for coordination rewards
                    agent_positions = {}
                    agent_directions = {}
                    for a in self._env_handler.get_agents_handle():
                        a_obj = self._env_handler.env.agents[a]
                        if a_obj.position is not None:
                            agent_positions[a] = a_obj.position
                            agent_directions[a] = a_obj.direction
                    ...
                    for agent in self._env_handler.get_agents_handle():
                        ...
                        # --------------------------------------------------------
                        # REWARD 2: Stationary penalty (escalating)
                        # --------------------------------------------------------
                        ...
                        # --------------------------------------------------------
                        # REWARD 4: Time pressure
                        # --------------------------------------------------------
                        shaped_reward -= 0.005
                        
                        # --------------------------------------------------------
                        # NEW REWARD 5: Coordination - Yielding Behavior
                        # Reward agents for yielding to higher-priority agents
                        # --------------------------------------------------------
                        if agent_obj.position is not None and num_agents > 1:
                            for other_agent in self._env_handler.get_agents_handle():
                                ...
                                dist_to_other = abs(agent_obj.position[0] - other_obj.position[0]) + \
                                               abs(agent_obj.position[1] - other_obj.position[1])
                                
                                # If very close (potential conflict)
                                if dist_to_other <= 2:
                                    # Higher handle = lower priority
                                    if agent > other_agent:
                                        ...
                                        if other_agent in self._prev_distances:
                                            other_prev = self._prev_distances.get(other_agent, 0)
                                            other_curr_dist = abs(other_obj.position[0] - other_obj.target[0]) + \
                                                             abs(other_obj.position[1] - other_obj.target[1])
                                            if other_prev > other_curr_dist:
                                                # Other agent made progress, reward this agent for not blocking
                                                shaped_reward += 0.05
                                    else:
                                        # This agent has priority - reward for moving
                                        if action_dict[agent] == 2:  # MOVE_FORWARD
                                            shaped_reward += 0.05
                        
                        # --------------------------------------------------------
                        # NEW REWARD 6: Team Completion Bonus
                        # When other agents complete, all active agents get small bonus
                        # --------------------------------------------------------
                        newly_completed = set()
                        for other_agent in self._env_handler.get_agents_handle():
                        ...
                        # Clip reward
                        # --------------------------------------------------------
                        shaped_reward = np.clip(shaped_reward, -1.5, 2.0)
