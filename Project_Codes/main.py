import os
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"  # Show only critical TF errors
...
def main(episodes: int, training: str, rendering: bool, checkpoint: Optional[str],
         synclog: bool, verbose: bool, seed, world):
    with open("parameters/setup.json") as json_file:
        ...
        if args.agent is not None:
            parameters['sys']['agent_class'] = args.agent
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-V", "--verbose",
        dest="verbose",
        help="Prints results on the console",
        default=False,
        action="store_true",
    )
    parser.add_argument(
    ...
    """,
        default="debug",
        choices=["debug", "eval", "tuning", "best", "fresh"],
    )
    parser.add_argument(
        "-C", "--checkpoint",
        dest="checkpoint",
    ...
    parser.add_argument(
        "-X", "--seed",
        dest="seed",
        help="Custom seed for current execution, overrides the defined setup",
        default=None,
    )
    parser.add_argument(
        "-W", "--world",
        dest="world",
        help="Custom environment for execution, overrides the defined setup",
        default=None,
    )
    parser.add_argument(
        "--agent",
        dest="agent",
        help="Select agent class",
        default=None,
    )
